				...
				<div class="navbar-inner">
					<ul class="nav navbar-nav">
						...
						<li class="dropdown active">
							...
							<ul class="dropdown-menu">
								<li class="active"><a href="<c:url value="/admin/images" />">Изображения</a></li>
	...
	<div id="textbox">
		<div class="row">
			<div class="col-sm-7">
				<div class="resource add-resource">
					<form action="${resourceUrl}/uploadImage" enctype="multipart/form-data" method="post">
						<div class="col-sm-2">
							<label for="image" class="control-label">Ново изображение:</label>
						</div>
						<div class="col-sm-offset-1 col-sm-6">
							<input type="file" class="form-control" name="image" id="image" size="50"/>
						</div>
						<div class="col-sm-3">
			...
			<div class="col-sm-5">
				<div class="resource count-selector">
					<form action="/app/admin/images/page/1">
						<div class="row">
							<div class="col-sm-4">
		...
		<c:forEach var="image" items="${images}">
			<div class="resource">
				<div class="row">
					<div class="col-sm-6">
						<img class="resource-image" src="<c:out value="${resourceUrl}/${image.name}"/>">
					</div>
					<br>
					<div class="col-sm-6">
						<div class="row">
							<div class="col-sm-12">
								<p>Име на файла: <strong><c:out value="${image.name}"/></strong></p>
							</div>
						</div>
						<div class="row">
							<div class="col-sm-9">
								<input type="text" class="form-control" id="<c:out value="text-for-button-${image.id}"/>" value="<c:out value="${resourceUrl}/${image.name}"/>" readonly>
							</div>
							<div class="col-sm-3">
								<button class="copy-text-button btn btn-default btn-block" id="<c:out value="${image.id}"/>">Копирай</button>
							</div>
						</div>
						<div class="resource-delete-button-offset">
						</div>
						<form action="${resourceUrl}/deleteImage" method="post" class="form-horizontal" onsubmit="return confirm('Наистина ли искате да изтриете този запис от системата?');">	
			...
			<div class="row">
				<div class="col-sm-3">
					<c:if test="${previousPageNumber != 0}">
						<a class="btn btn-info btn-lg btn-block" href="/app/admin/images/page/<c:out value="${previousPageNumber}"/>?count=<c:out value="${count}"/>"><span class="glyphicon glyphicon-chevron-left"></span> Назад</a>
					</c:if>
				</div>
				<div class="col-sm-offset-6 col-sm-3">
					<c:if test="${nextPageNumber != 0}">
						<a class="btn btn-info btn-lg btn-block" href="/app/admin/images/page/<c:out value="${nextPageNumber}"/>?count=<c:out value="${count}"/>">Напред <span class="glyphicon glyphicon-chevron-right"></span></a>
					</c:if>
				</div>
			</div>
		</div>
